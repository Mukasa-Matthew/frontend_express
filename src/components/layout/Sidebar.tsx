        ...
        {navigationItems.map((item) => {
          ...
          return (
            <Button
              ...
              variant="ghost"
              className={cn(
                "w-full justify-start text-left h-11 hover:bg-slate-100 hover:text-slate-900 text-slate-700 transition-colors",
                isActive && "bg-slate-100 text-slate-900 font-semibold"
              )}
              asChild
            >
              <Link to={item.href} className="flex items-center space-x-3" onClick={() => setIsOpen(false)}>
